...
function verify() {
    ...
    //三 设置连接信息
    //d第一个参数http请求方式，支持所有http的请求方式主要使用g
    // et post
...
function callback() {
    ...
    if (xmlhttp.readyState == 4) {
        ...
        if (xmlhttp.status == 200) {
            //使用responseXMLDOM的方式来接受xml数据对象的DOM对象
            var domObj = xmlhttp.responseXML;
            //<message>123123</message>
            //利用getElementByTagName可以根据标签来获取元素节点，返回的是一个数据
            var messageNodes = domObj.getElementsByTagName("message");

            if (messageNodes.length > 0) {
                //获取message节点的文本内容
                var textNodes = messageNodes[0].firstChild;
                //对于文本节点，可以通过nodeValue的方式返回文本节点的文本内容
                var responseMsgText = textNodes.nodeValue;
                //将数据显示在页面上
